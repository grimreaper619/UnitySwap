...
contract UnitySwap is Context, IERC20, Ownable {
    ...
    function setWallets(address foundation, address charity)
        external
        onlyOwner
    {
    ...
    function _takeCharityAndFoundation(uint256 tCharity, uint256 tfoundation)
        private
    {
        uint256 currentRate = _getRate();
        uint256 rCharity = tCharity.mul(currentRate);
    ...
    function calculateFoundationFee(uint256 _amount)
        ...
        returns (uint256)
    {
        return _amount.mul(_foundationFee).div(10**4);
    }
    ...
    function _transferStandard(
        ...
        _takeCharityAndFoundation(
            calculateCharityFee(tAmount),
            calculateFoundationFee(tAmount)
        );
