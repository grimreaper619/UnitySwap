...
contract("UnityTest", (accounts) => {
  ...
  describe("Standard Functions", () => {
    ...
    it("should transfer tokens normally without fee", async () => {
      ...
      assert.equal(await this.token.balanceOf(accounts[0]),BigInt(prevBalance) - BigInt(1e16));
    ...
    it("should transfer from another wallet with allowance without fee", async () => {
      await this.token.transferFrom(accounts[0],accounts[2],1e11, {from: accounts[1]});
      assert.equal(await this.token.balanceOf(accounts[2]),1e11);
      assert.equal(await this.token.allowance(accounts[0], accounts[1]),1e12-1e11);
    });
  });
  describe("Tax Functions", () => {
    ...
    it("should not allow transaction above max Tx amount", async () => {
      await expectRevert(this.token.transfer(accounts[2],2e15, {from: accounts[1]}),
    ...
    it("should give fees to charity and foundation wallet", async () => {
      [charity, foundation] = [await this.token._charityWallet(),await this.token._foundationWallet()];
      ...
      assert.equal(Number(await this.token.balanceOf(charity)), prevCharity+25e6) //0.25%
      assert.equal(Number(await this.token.balanceOf(foundation)), prevFoundation+100e6) //1%
    });
  });
  describe("Reflection Transfer", () => {
    ...
    it("should transfer token from an excluded wallet", async() => {
      ...
      assert.equal(Number(await this.token.balanceOf(accounts[1])), prevBal - 1e10); //No reflection received
    });

    it("should transfer token to an excluded wallet", async () => {
      const prevBal = Number(await this.token.balanceOf(accounts[1]));
      await this.token.transfer(accounts[1], 1e8, {from: accounts[4]});
      assert.equal(Number(await this.token.balanceOf(accounts[1])), prevBal +(1e8 - 1e8 * 0.035)); //No reflection 
    });
    ...
    it("should transfer between two excluded wallets", async () => {
      ...
      assert.equal(Number(await this.token.balanceOf(accounts[1])), prevBal1 - 1e10);
      assert.equal(Number(await this.token.balanceOf(accounts[2])), prevBal2 + (1e10 - 1e10 * 0.035));
    });
   ...
   describe("Token trading", () => {
     it("should buy token from AMM", async () => {
        ...
        const output = await this.router.getAmountsOut(
          1e12,
          path
        );
        ...
        assert.isAtMost(expected - expected * 0.035, Number(await this.token.balanceOf(accounts[5])));
     });
     ...
     it("should sell token to AMM", async () => {
      ...
      await this.token.approve(this.router.address,1e14,{from: accounts[5]});
      ...
      assert.isAtMost(expected - expected * 0.035, Number(await web3.eth.getBalance(accounts[6])) - Number(prevBal));
     });

     it("should trigger auto liquidity addition", async () => {
      await this.token.transfer(this.token.address,BigInt(1e15),{from: accounts[0]});
   ...
   describe("Owner Privilage", async () => {
      ...
      it("should set new router address", async () => {
        try {
          await this.token.updateRouter('0xD99D1c33F9fC3444f8101754aBC46c52416550D1');
        } catch (err) {
          assert.equal(err, null);
        }
      });
      ...
      it("should claim any stuck tokens/bnb", async () => {
        try {
          await this.token.claimStuckTokens('0x0000000000000000000000000000000000000000');
          await this.token.claimStuckTokens('0x77c21c770Db1156e271a3516F89380BA53D594FA');
        } catch (err) {
          assert.equal(err, null);
        }
      });
