...
contract("AmnestyTest", (accounts) => {
  before(async () => {
    this.token = await Amnesty.deployed();
  ...
  describe("Metadata", () => {
    it("should return the correct name", async () => {
      ...
      assert.equal(name, "Amnesty International");
    });

    it("should return the correct sumbol", async () => {
      const symbol = await this.token.symbol();
      assert.equal(symbol, "AMNESTY");
    });
    ...
    it("should return the fixed total supply", async () => {
      const supply = await this.token.totalSupply();
      assert.equal(supply, 10 * 10 ** 6 * 10 ** 9);
    });
  });
  describe("Standard Functions", () => {
    ...
    it("should transfer tokens normally without fee", async () => {
      ...
      await this.token.transfer(accounts[1], BigInt(1e15));
      assert.equal(
        await this.token.balanceOf(accounts[0]),
        BigInt(prevBalance) - BigInt(1e15)
      );
      assert.equal(await this.token.balanceOf(accounts[1]), BigInt(1e15));
    });
  ...
  describe("Tax Functions", () => {
    ...
    it("should not allow transaction above max Tx amount", async () => {
      await expectRevert(
        this.token.transfer(accounts[2], 6e13, { from: accounts[1] }),
    ...
    it("should transfer tokens with tax", async () => {
      ...
      assert.equal(await this.token.balanceOf(accounts[3]), 1e5 - 2500); //2.5% fee
    });
  ...
  describe("Token trading", () => {
    ...
    it("should sell token to AMM", async () => {
      ...
      const output = await this.router.getAmountsOut(1e8, path);
